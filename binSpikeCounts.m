...
%   Inputs:
%       S: 
%           1x1 struct contains 2 fields, where S.X is kinematic data in
%           (n_time, 6), and S.t is time stamp
%       UnitSpikes: 
%           A struct in (n_channel x max_unit, 1), containing 1
%           field. UnitSpikes(i).t is a double matrix.
%       params:
%          
%   Output:
%       R:
%           a double array in (n_bin, n_channel x max_unit)
%       Xout:
%           a double array in (n_bin, 6)
...
for iTrial = 1:Ntrials
    if strcmp(params.BINMETHOD,'slidingwindow')
        ...
        edges = S(iTrial).t(1):binsize:S(iTrial).t(end); % 窗的端点
